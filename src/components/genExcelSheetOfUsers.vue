<template>
  <div>
    gen excel sheet of users

    <span v-if="downFinish == false">preparing data to be downloaded ... Please Wait ...</span>
    <download-excel
      v-else
    ...
    beforeMount(){
      ...
      this.$store.state.db.db.ref('userAuthDetail/')
        .once('value',function (snapshot) {
          ...
          for(let i in userAuthDetail){
            //console.log(userAuthDetail[i])

            //get membership details
            vm.$store.state.db.db.ref('membershipDetail/' + userAuthDetail[i].uid)
              .once('value',function (snapshot2) {
                ...
                if(userMemDetail == null) {
                  ...
                  tmpObj = {
                    //auth
                    uid: userAuthDetail[i].uid,
                    name: userAuthDetail[i].name,
